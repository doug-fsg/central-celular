...
} from './DashboardResumoDetailModal.vue'
...
const cards = computed(() => {
  ...
  return [
    {
      key: 'sem-cuidador' as const,
      label: DASHBOARD_UNIFIED_COPY.miniStats.semCuidador.label,
      hint: DASHBOARD_UNIFIED_COPY.miniStats.semCuidador.hint(t.membrosSemCuidador),
    ...
    {
      key: 'rede-incompleta' as const,
      label: DASHBOARD_UNIFIED_COPY.miniStats.redeIncompleta.label,
      hint: DASHBOARD_UNIFIED_COPY.miniStats.redeIncompleta.hint(t.celulasBaixaCobertura),
      value: t.celulasBaixaCobertura,
    ...
    {
      key: 'acompanhados' as const,
      label: DASHBOARD_UNIFIED_COPY.miniStats.acompanhados.label,
      hint: DASHBOARD_UNIFIED_COPY.miniStats.acompanhados.hint(r.comCuidador),
      value: r.comCuidador,
    ...
    {
      key: 'total-membros' as const,
      label: DASHBOARD_UNIFIED_COPY.miniStats.totalMembros.label,
      hint: DASHBOARD_UNIFIED_COPY.miniStats.totalMembros.hint,
...
const modalCelulas = computed(() => {
  if (!props.listas || activeKind.value !== 'rede-incompleta') return []
  return props.listas.celulasBaixaCobertura
})

function openDetail(kind: ResumoDetailKind) {
...
<template>
  ...
  <div v-else class="grid h-full grid-cols-2 gap-3">
    <button
      ...
      :key="card.key"
      type="button"
      class="relative flex min-h-[132px] flex-col rounded-2xl border p-4 text-left shadow-sm transition-colors hover:border-primary-200 hover:shadow-md touch-manipulation"
